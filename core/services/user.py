    ...
    def _check_email_by_emailhunter(email: str) -> None:
        ...
        if resp.status_code == 200:
            ...
            if rest_status not in settings.EMAIL_VERIFY_VALID_STATUS:
                raise HTTPException(status_code=422, detail=f"Your email has <{rest_status}> status by emailhunter.co")
            if rest_result not in settings.EMAIL_VERIFY_VALID_RESULT:
                raise HTTPException(status_code=422, detail=f"Your email has <{rest_result}> result by emailhunter.co")
